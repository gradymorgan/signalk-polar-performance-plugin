...
- One input (observed on `environment.wind.speedTrue`) could occasionally never receive its first delta after `start()` — e.g. a subscription race, or an upstream source (a wind calculator needing GPS/apparent-wind/boat-speed first) that hadn't started publishing yet — leaving that input stuck at no data indefinitely even while the plugin's other inputs subscribed fine in the same `start()` call. A restart of the plugin always cleared it, but a fixed grace period isn't enough since the source can start publishing at an arbitrary time after boot. Added a subscription watchdog that resubscribes any input still without data — quickly at first, backing off to a steady 30s poll, retried indefinitely — so it recovers on its own whenever the source starts publishing, with a one-time status note (not a hard stop) if it's been stuck for a long while.
